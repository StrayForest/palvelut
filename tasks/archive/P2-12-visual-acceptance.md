...
- Added the documented provider CTA state to the public home surface.
...
- Browser acceptance checks horizontal overflow at every required width, keeps the mobile search surface above the initial fold and verifies visible keyboard focus on the provider CTA.
...
- Final documentation exact-head must pass the same Compose workflow before merge.
