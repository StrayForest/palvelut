...
class ProviderAdmin(admin.ModelAdmin):
    ...
    def suspend_selected(self, request: HttpRequest, queryset: QuerySet[Provider]) -> None:
        changed = 0
        for provider in queryset:
            try:
                suspend_provider(provider=provider, actor=request.user, reason="Staff admin action")
            except ValidationError as exc:
                self.message_user(request, "; ".join(exc.messages), level=messages.ERROR)
            else:
                changed += 1
        if changed:
            self.message_user(request, f"Suspended {changed} provider(s).", level=messages.SUCCESS)

    @admin.action(description="Merge exactly two duplicates (oldest UUIDv7 survives)")
    def merge_selected_duplicate_pair(self, request: HttpRequest, queryset: QuerySet[Provider]) -> None:
        providers = list(queryset.order_by("id")[:3])
        if len(providers) != 2:
            self.message_user(request, "Select exactly two providers to merge.", level=messages.ERROR)
            return
        try:
            survivor = merge_duplicate_pair(first=providers[0], second=providers[1], actor=request.user)
        except ValidationError as exc:
            self.message_user(request, "; ".join(exc.messages), level=messages.ERROR)
            return
        self.message_user(request, f"Merged duplicate into {survivor.display_name}.", level=messages.SUCCESS)
...
class ProfileRevisionAdmin(admin.ModelAdmin):
    ...
    def revision_diff_view(self, obj: ProfileRevision | None) -> str:
        ...
        rendered = json.dumps(revision_diff(obj), ensure_ascii=False, indent=2, sort_keys=True)
        return format_html("<pre>{}</pre>", rendered)

    @admin.action(description="Approve selected pending revisions")
    def approve_selected(self, request: HttpRequest, queryset: QuerySet[ProfileRevision]) -> None:
        changed = 0
        for revision in queryset:
            try:
                ...
                changed += 1
        if changed:
            self.message_user(request, f"Approved {changed} revision(s).", level=messages.SUCCESS)

    @admin.action(description="Request changes for selected pending revisions")
    def request_changes_selected(self, request: HttpRequest, queryset: QuerySet[ProfileRevision]) -> None:
        changed = 0
        for revision in queryset:
            try:
                ...
                changed += 1
        if changed:
            self.message_user(request, f"Requested changes for {changed} revision(s).", level=messages.SUCCESS)


@admin.register(ProviderClaim)
class ProviderClaimAdmin(admin.ModelAdmin):
    list_display = ("provider", "claimant", "status", "evidence_type", "requested_at", "reviewed_at")
    list_filter = ("status", "evidence_type")
    search_fields = ("provider__display_name", "provider__legal_name", "claimant__username")
    ...
    def approve_selected(self, request: HttpRequest, queryset: QuerySet[ProviderClaim]) -> None:
        changed = 0
        for claim in queryset:
            try:
                ...
                changed += 1
        if changed:
            self.message_user(request, f"Approved {changed} claim(s).", level=messages.SUCCESS)

    @admin.action(description="Reject selected pending claims")
    def reject_selected(self, request: HttpRequest, queryset: QuerySet[ProviderClaim]) -> None:
        changed = 0
        for claim in queryset:
            try:
                ...
                changed += 1
        if changed:
            self.message_user(request, f"Rejected {changed} claim(s).", level=messages.SUCCESS)


@admin.register(VerificationCheck)
class VerificationCheckAdmin(admin.ModelAdmin):
    list_display = ("provider", "kind", "status", "checked_by", "checked_at", "expires_at")
