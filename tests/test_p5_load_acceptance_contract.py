...
def test_load_probe_enforces_server_side_slos_and_overload() -> None:
    ...
    assert 'warm_path = "/fi/"' not in script
    assert 'search_path = "/fi/search/?q=Performance"' not in script
...
def test_workflow_records_bounded_pool_and_queue_evidence() -> None:
    ...
    assert "cache_pool_max_connections=16" in workflow
    assert "gunicorn_backlog=128" in workflow
