...
def test_authenticated_requests_cannot_enter_cdn_cache() -> None:
    ...
    assert 'len(http.request.headers["authorization"][0]) gt 0' in bypass["expression"]
    ...
    assert 'len(http.request.headers["authorization"][0]) eq 0' in public["expression"]
    assert 'len(http.request.headers["cookie"][0]) eq 0' in public["expression"]

    for protected_prefix in ("/palvelut/account/", "/palvelut/staff/", "/palvelut/report/"):
